...
const CommitteeMemberLogin =(props) => {
  let [Chairman, setChairman] = useState({username: "", password:""})
  const navigate = useNavigate();
  const handleSubmit = async (e) => {
      e.preventDefault();
      const response = await fetch("http://localhost:8000/api/auth/ChairmanLogin", {
          method: 'POSt',

          headers: {
              'Content-Type': 'application/json'
               },
               body: JSON.stringify({username:Chairman.username,password:Chairman.password })
  ...
  return (
    ...
    <form className="App" onSubmit={handleSubmit}>
        ...
        <input type="text"  placeholder='UserName' className="form-control" value={Chairman.username} onChange={onChange} name="username" id="username" aria-describedby="IdHelp"/>
        <div id="IdHelp" className="form-text">Never share your Credentials with anyone else.</div>
        </div>
        <div className="mb-3">
        <label for="exampleInputPassword1" className="form-label">Password</label>
        <input type="password"  value={Chairman.password} onChange={onChange} name="password" placeholder='Password' className="form-control" id="password" />
        </div>
...
export default CommitteeMemberLogin
